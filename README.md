# TryHackMe — Suivi de Progression
**Compte :** devlabcyb  
**Objectif :** Autoformation amelioree
**Co_Worker :** LLM | CLAUDE Sonnet 4.6  

---

## Statut Global

| Path | Niveau | Statut | Completion |
|------|--------|--------|------------|
| Pre Security | Debutant | 🔄 En cours | 1% |
| Security Engineer | Easy | ⏳ A venir | 0% |
| DevSecOps | Intermediate | ⏳ A venir | 0% |
| Attacking and Defending AWS | Intermediate | ⏳ A venir | 0% |

---

## PATH 1 — Pre Security

> Prerequis recommande avant Security Engineer Path

### Module 1 — Introduction to Cyber Security

| Room | Statut | Debut | Fin | Apport personnel | Valeur equipe |
|------|--------|-------|-----|-----------------|---------------|
| Offensive Security Intro | ⏳ | - | - | - | - |
| Defensive Security Intro | ⏳ | - | - | - | - |
| Careers in Cyber | ⏳ | - | - | - | - |

### Module 2 — Network Fundamentals

| Room | Statut | Debut | Fin | Apport personnel | Valeur equipe |
|------|--------|-------|-----|-----------------|---------------|
| What is Networking? | ⏳ | - | - | - | - |
| Intro to LAN | ⏳ | - | - | - | - |
| OSI Model | ⏳ | - | - | - | - |
| Packets & Frames | ⏳ | - | - | - | - |
| Extending the Network | ⏳ | - | - | - | - |

### Module 3 — How The Web Works

| Room | Statut | Debut | Fin | Apport personnel | Valeur equipe |
|------|--------|-------|-----|-----------------|---------------|
| DNS in Detail | ⏳ | - | - | - | - |
| HTTP in Detail | ⏳ | - | - | - | - |
| How Websites Work | ⏳ | - | - | - | - |
| Putting it all Together | ⏳ | - | - | - | - |

### Module 4 — Linux Fundamentals

| Room | Statut | Debut | Fin | Apport personnel | Valeur equipe |
|------|--------|-------|-----|-----------------|---------------|
| Linux Fundamentals Part 1 | ⏳ | - | - | - | - |
| Linux Fundamentals Part 2 | ⏳ | - | - | - | - |
| Linux Fundamentals Part 3 | ⏳ | - | - | - | - |

### Module 5 — Windows Fundamentals

| Room | Statut | Debut | Fin | Apport personnel | Valeur equipe |
|------|--------|-------|-----|-----------------|---------------|
| Windows Fundamentals 1 | ⏳ | - | - | - | - |
| Windows Fundamentals 2 | ⏳ | - | - | - | - |
| Windows Fundamentals 3 | ⏳ | - | - | - | - |

---

## PATH 2 — Security Engineer

> A commencer apres Pre Security

### Room 1 — Introduction to Security Engineering

| Task | Titre | Statut | Debut | Fin | Apport personnel | Valeur equipe |
|------|-------|--------|-------|-----|-----------------|---------------|
| Task 1 | Introduction | ⏳ | - | - | - | - |
| Task 2 | What is a Security Engineer? | ⏳ | - | - | - | - |
| Task 3 | Core Responsibilities | ⏳ | - | - | - | - |
| Task 4 | Continuous Improvement | ⏳ | - | - | - | - |
| Task 5 | Additional Roles and Responsibilities | ⏳ | - | - | - | - |
| Task 6 | Walking in Their Shoes | ⏳ | - | - | - | - |
| Task 7 | Conclusion | ⏳ | - | - | - | - |

---

## Legende

| Icone | Signification |
|-------|---------------|
| ✅ | Complete |
| 🔄 | En cours |
| ⏳ | A venir |
| ⚠️ | Bloque / Question |

---

## Lien avec DEVLABCYB

| Chapitre DEVLABCYB | Rooms THM complementaires |
|--------------------|--------------------------|
| Ch4 — Monitoring & Detection | Intro to SIEM, Wazuh, Splunk |
| Ch5 — Cloud & DevSecOps | Attacking and Defending AWS, DevSecOps Path |

---

*Derniere mise a jour : 10/04/2026*
